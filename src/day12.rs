...
pub fn part1_chars(input: &str) -> i32 {
    ...
    // graph_hm.insert("start".to_string(), graph);
...
fn print_permutations(
    ...
    if *graph.node_weight(node_index).unwrap() == "end" {
        ...
        for visited_nodes in visited_ids.iter() {
            // print!("{} ", visited_nodes)
        }
        // println!("end");
        return;
    ...
    // let mut dfs = WalkNeighbors::new(&graph, node_index);
    ...
    //     if neighbour.id.chars().any(|x| x.is_uppercase())
    //         || !visited_ids.contains(neighbour.id.as_str())
    //     {
    //         let mut new_visited_ids: HashSet<String> = visited_ids.clone();
    //         new_visited_ids.insert(neighbour.id.to_string());
    //         print_permutations(node, new_visited_ids);
    //     }
    // }
}
